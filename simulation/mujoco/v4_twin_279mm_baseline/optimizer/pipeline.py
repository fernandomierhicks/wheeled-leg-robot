"""pipeline.py — S1→S10 automated tuning pipeline.
...
STEPS = [
    ("S1", "v4_twin_279mm_baseline.optimizer.optimize_lqr",
     "s01_lqr_pitch_step",
     ["Q_PITCH", "Q_PITCH_RATE", "Q_VEL", "R"],
     None, False),

    ("S2", "v4_twin_279mm_baseline.optimizer.optimize_lqr",
     "s02_leg_height_gain_sched",
     ["Q_PITCH", "Q_PITCH_RATE", "Q_VEL", "R"],
     "S1", True),

    ("S3", "v4_twin_279mm_baseline.optimizer.optimize_vel_pi",
     "s03_vel_pi_disturbance",
     ["KP_V", "KI_V", "KFF_V"],
     None, False),

    ("S4", "v4_twin_279mm_baseline.optimizer.optimize_vel_pi",
     "s04_vel_pi_staircase",
     ["KP_V", "KI_V", "KFF_V"],
     "S3", False),

    ("S5", "v4_twin_279mm_baseline.optimizer.optimize_vel_pi",
     "s05_vel_pi_leg_cycling",
     ["KP_V", "KI_V", "KFF_V"],
     "S4", True),

    ("S6", "v4_twin_279mm_baseline.optimizer.optimize_yaw_pi",
     "s06_yaw_pi_turn",
     ["KP_YAW", "KI_YAW"],
     None, False),

    ("S7", "v4_twin_279mm_baseline.optimizer.optimize_yaw_pi",
     "s07_drive_turn",
     ["KP_YAW", "KI_YAW"],
     "S6", True),

    ("S8", "v4_twin_279mm_baseline.optimizer.optimize_suspension",
     "s08_terrain_compliance",
     ["LEG_K_S", "LEG_B_S", "LEG_K_ROLL", "LEG_D_ROLL"],
     None, True),

    ("S9", "v4_twin_279mm_baseline.optimizer.optimize_integrated",
     "s09_integrated",
     ["Q_PITCH", "Q_PITCH_RATE", "Q_VEL", "R",
    ...
    ("S10", "v4_twin_279mm_baseline.optimizer.optimize_integrated",
     "s10_jump",
     ["JUMP_CROUCH_TIME", "JUMP_MAX_TORQUE", "JUMP_RAMP_UP_S", "JUMP_RAMP_DOWN_RAD"],
     None, True),
]
...
_DEFAULTS_MAP = {
    ...
    "JUMP_CROUCH_TIME":  ("gains", "jump", "crouch_time"),
    "JUMP_MAX_TORQUE":   ("gains", "jump", "max_torque"),
    "JUMP_RAMP_UP_S":    ("gains", "jump", "ramp_up_s"),
    "JUMP_RAMP_DOWN_RAD":("gains", "jump", "ramp_down_rad"),
}
...
def _random_seed(param_keys: list[str]) -> dict:
    ...
    import numpy as np
    from v4_twin_279mm_baseline.optimizer.search_space import INTEGRATED_SPACE
...
def _read_best_from_csv(scenario: str, param_keys: list[str]) -> dict | None:
    ...
    from v4_twin_279mm_baseline.optimizer.run_log import get_scenario_csv_path, load_best_params
...
def _save_baseline(step_id: str, best: dict, fitness: float = 0.0):
    ...
    from v4_twin_279mm_baseline.optimizer.baseline import save_baseline_gains
    save_baseline_gains(
        best_params=best,
...
def _run_step(step_id: str, optimizer_module: str, scenario: str,
    ...
    from v4_twin_279mm_baseline.optimizer.run_log import get_scenario_csv_path
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="S1→S10 automated tuning pipeline.",
