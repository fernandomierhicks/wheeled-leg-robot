...
Parameters can use log10 sampling (legacy design quantities) or normalized
linear sampling (signed/zero-inclusive firmware gains). Parameters whose
optimal value can be exactly zero can be marked via `zero_ok`.
"""
...
class ParamSpec:
    ...
    lo: float                   # lower bound in physical units
    ...
    def random_init(self, rng: np.random.Generator | None = None) -> Dict[str, float]:
        ...
        for name, spec in self.params.items():
            if spec.scale == "linear":
                candidate[name] = float(rng.uniform(spec.lo, spec.hi))
            else:
    ...
    def sample_offspring(
        ...
        for name, spec in self.params.items():
            if spec.scale == "linear":
                ...
                val = parent[name] + rng.normal(0.0, sigmas[name] * span)
                child[name] = max(spec.lo, min(spec.hi, val))
                continue
...
JUMP_SPACE = SearchSpace(params={
    "JUMP_CROUCH_TIME":  ParamSpec(0.2, 5.0),
    "JUMP_MAX_TORQUE":   ParamSpec(1.0, 12.0),
    "JUMP_RAMP_UP_S":    ParamSpec(0.001, 0.1),
    "JUMP_RAMP_DOWN_RAD":ParamSpec(0.02, 0.5),
})
