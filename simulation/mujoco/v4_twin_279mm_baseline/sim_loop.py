...
from v4_twin_279mm_baseline.physics import (build_xml, build_assets, solve_ik,
...
from v4_twin_279mm_baseline.controllers.velocity_pi import VelocityPI
from v4_twin_279mm_baseline.controllers.yaw_pi import YawPI
from v4_twin_279mm_baseline.controllers.hip import (
    hip_position_torque, hip_impedance_torque, roll_leveling_offsets,
    knee_spring_torque,
)
from v4_twin_279mm_baseline.controllers.jump import JumpController, RobotMode
from v4_twin_279mm_baseline.twin.firmware_control import (
    ControlInput as FirmwareControlInput, FirmwareController,
)
...
class SimController:
    ...
    def _init_controllers(self, params, model, data=None):
        ...
        self._m_b = (robot.body_mass_excluding_wheels
    ...
    def _tick_firmware_balance(
        ...
        alpha = hip_q_to_alpha(hip_q_avg, robot)
        alpha_target = hip_q_to_alpha(q_hip_target, robot)
        ...
        firmware.params["roll_ctrl_en"] = float(bool(use_roll_leveling))
        if not use_ff1:
            firmware.params["ff1_alpha"] = 0.0
        if not use_ff2:
        ...
        fell = abs(pitch_true) > params.thresholds.fall_rad or output.fault is not None
        return dict(
            ...
            q_nom_L=q_nom_L, q_nom_R=q_nom_R, v_batt=self.v_batt,
            ...
            pitch_delayed=float(pitch_delayed),
            pitch_rate_delayed=float(pitch_rate_delayed),
            wheel_vel_delayed=float(wheel_vel_delayed),
            pitch_predicted=float(pitch_delayed),
            pitch_rate_predicted=float(pitch_rate_delayed),
            wheel_vel_predicted=float(wheel_vel_delayed),
            tau_cmd_L=tau_l_cmd, tau_cmd_R=tau_r_cmd,
            tau_delayed_L=float(tau_l_delayed), tau_delayed_R=float(tau_r_delayed),
