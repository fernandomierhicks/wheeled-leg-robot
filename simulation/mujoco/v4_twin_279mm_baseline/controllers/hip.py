...
from v4_twin_279mm_baseline.params import (RobotGeometry, HipMotorParams,
...
def roll_leveling_offsets(roll: float, roll_rate: float,
    ...
    # Clamp final setpoints to full joint range — the roll offset can
    # push toward the limits but never beyond Q_EXT / Q_RET.
...
if __name__ == "__main__":
    ...
    q_L, q_R = roll_leveling_offsets(
        ...
        q_hip_sym=robot.Q_NOM, suspension=susp, robot=robot)
    assert q_L >= robot.Q_EXT and q_L <= robot.Q_RET, "Left setpoint out of joint range!"
    assert q_R >= robot.Q_EXT and q_R <= robot.Q_RET, "Right setpoint out of joint range!"
    print(f"  Clamp test:  roll=90° -> q_L={math.degrees(q_L):.1f}° "
          f"q_R={math.degrees(q_R):.1f}° (within joint range)")

    # Verify Q_RET and Q_EXT are reachable with zero roll
    q_L, q_R = roll_leveling_offsets(0.0, 0.0, robot.Q_RET, susp, robot)
    assert abs(q_L - robot.Q_RET) < 1e-6, f"Q_RET not reachable! got {q_L}"
    q_L, q_R = roll_leveling_offsets(0.0, 0.0, robot.Q_EXT, susp, robot)
    assert abs(q_L - robot.Q_EXT) < 1e-6, f"Q_EXT not reachable! got {q_L}"
    print(f"  Endpoint test: Q_RET and Q_EXT reachable with zero roll")
