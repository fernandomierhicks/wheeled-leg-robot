...
DRIVE_RAMP_S        = 0.20
MAX_SPEED_PITCH     = 0.55   # max lean angle for top-speed ramp (rad)
MAX_SPEED_RAMP_S    = 3.0    # time to ramp from 0 → MAX_SPEED_PITCH (sim seconds)
...
def _plot_process(q: mp.Queue, cmd_q: mp.Queue, window_s: float) -> None:
    ...
    ax_fwd    = fig.add_axes([0.05, 0.210, 0.27, 0.042])
    ax_maxspd = fig.add_axes([0.34, 0.210, 0.32, 0.042])
    ax_bwd    = fig.add_axes([0.68, 0.210, 0.27, 0.042])

    bw, gap = 0.162, 0.010
    bx = [0.05 + i * (bw + gap) for i in range(5)]
    ...
    ax_clear = fig.add_axes([bx[4], 0.158, bw, 0.042])
    ...
    BTN_MAXSPD = "#1a3a5a"
    BTN_CLEAR  = "#3a1a5a"
    btn_fwd    = Button(ax_fwd,    "Fwd >>",      color=BTN_COL,   hovercolor=BTN_HOV)
    btn_maxspd = Button(ax_maxspd, "MAX SPD ▶▶",  color=BTN_MAXSPD, hovercolor="#2a6a9a")
    btn_bwd    = Button(ax_bwd,    "<< Bwd",      color=BTN_COL,   hovercolor=BTN_HOV)
    ...
    btn_clear  = Button(ax_clear,  "Clear Arena", color=BTN_CLEAR,  hovercolor="#6a2a9a")
    ...
    def _cancel_max_spd():
        if max_spd_active[0]:
            max_spd_active[0] = False
            ax_maxspd.set_facecolor(BTN_MAXSPD)
            _send("MAX_SPEED_OFF")

    def _toggle_max_spd(_=None):
        if max_spd_active[0]:
            max_spd_active[0] = False
            ax_maxspd.set_facecolor(BTN_MAXSPD)
            _send("MAX_SPEED_OFF")
            ...
            max_spd_active[0] = True
            ax_maxspd.set_facecolor(BTN_ACTV)
            _send("MAX_SPEED_ON")
        fig.canvas.draw_idle()

    btn_maxspd.on_clicked(_toggle_max_spd)
    ...
    def _toggle_clear(_=None):
        arena_cleared[0] = not arena_cleared[0]
        ax_clear.set_facecolor(BTN_ACTV if arena_cleared[0] else BTN_CLEAR)
        btn_clear.label.set_text("Arena OFF" if arena_cleared[0] else "Clear Arena")
        _send("CLEAR_ARENA_TOGGLE")
        fig.canvas.draw_idle()
    ...
    btn_clear.on_clicked(_toggle_clear)

    def on_key_press(event):
        k = event.key
        if   k == 'up':    _cancel_max_spd(); driving[0] = 1;  ax_fwd.set_facecolor(BTN_ACTV); fig.canvas.draw_idle(); _send("DRIVE_FWD_ON")
        elif k == 'down':  _cancel_max_spd(); driving[0] = -1; ax_bwd.set_facecolor(BTN_ACTV); fig.canvas.draw_idle(); _send("DRIVE_BWD_ON")
        elif k == 'left':  _send("TURN_LEFT_ON")
        elif k == 'right': _send("TURN_RIGHT_ON")
        elif k == 'm':     _toggle_max_spd()
    ...
    fig.text(0.5, 0.018,
             "↑ / ↓ drive   ← / → turn   M = max speed   (click this window first)",
    ...
    while plt.fignum_exists(fig.number):
        ...
        for item in items:
            ...
            if item == "RESET":
                ...
                ax_fwd.set_facecolor(BTN_COL); ax_bwd.set_facecolor(BTN_COL)
                ax_maxspd.set_facecolor(BTN_MAXSPD)
                ax_clear.set_facecolor(BTN_CLEAR)
                btn_clear.label.set_text("Clear Arena")
                driving[0] = 0; max_spd_active[0] = False; arena_cleared[0] = False
...
def run_viewer():
    ...
    # ── Motor models (back-EMF derating + electrical lag + bearing drag) ──────
    ...
    # ── Arena geom IDs (walls + platforms + ramps) — for Clear Arena toggle ──
    _arena_prefixes = ('wall_', 'plat_', 'ramp_')
    _arena_geom_ids = [
        i for i in range(model.ngeom)
        if (n := mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_GEOM, i))
        and any(n.startswith(p) for p in _arena_prefixes)
    ]
    _arena_contype_orig  = {i: int(model.geom_contype[i])   for i in _arena_geom_ids}
    _arena_conaff_orig   = {i: int(model.geom_conaffinity[i]) for i in _arena_geom_ids}
    _arena_rgba_orig     = {i: model.geom_rgba[i].copy()    for i in _arena_geom_ids}
    arena_clear          = False
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        def _reset_state():
            ...
            nonlocal _throttle_moved, max_speed_active, max_speed_start_t, arena_clear
            _init()
            pitch_integral = 0.0; odo_x = 0.0
            wheel_pos_L = 0.0; wheel_pos_R = 0.0
            ...
            _throttle_moved = False; max_speed_active = False; max_speed_start_t = 0.0
            for _m in (motor_hip_L, motor_hip_R, motor_whl_L, motor_whl_R):
                _m.reset()
            # Restore arena geoms if they were cleared
            if arena_clear:
                for i in _arena_geom_ids:
                    model.geom_contype[i]    = _arena_contype_orig[i]
                    model.geom_conaffinity[i] = _arena_conaff_orig[i]
                    model.geom_rgba[i]       = _arena_rgba_orig[i]
                arena_clear = False
        ...
        while viewer.is_running():
            ...
            while True:
                try:
                    ...
                    if cmd == "RESTART":
                        ...
                        drive_pitch_target = 0.0
                    elif cmd == "MAX_SPEED_ON":
                        max_speed_active = True; max_speed_start_t = sim_t; odo_x = 0.0
                    elif cmd == "MAX_SPEED_OFF":
                        max_speed_active = False; drive_pitch_target = 0.0
                    elif cmd == "CLEAR_ARENA_TOGGLE":
                        arena_clear = not arena_clear
                        for i in _arena_geom_ids:
                            if arena_clear:
            ...
            for _ in range(n_steps):
                ...
                # Integrate wheel velocities to get wheel position (for LQR state)
                wheel_pos_L += data.qvel[d_whl_L] * model.opt.timestep
                wheel_pos_R += data.qvel[d_whl_R] * model.opt.timestep
                ...
                if max_speed_active and grounded:
                    elapsed = sim_t - max_speed_start_t
                    drive_pitch_target = min(
                        MAX_SPEED_PITCH,
                        elapsed / MAX_SPEED_RAMP_S * MAX_SPEED_PITCH)
                ...
                data.ctrl[3] = motor_whl_R.step(u_bal + turn_offset, data.qvel[d_whl_R], _dt)
            ...
            # Z fixed at 0.30 so the view stays level when jumping / falling
            ...
            if max_speed_active:
                vel_ms = abs((data.qvel[d_whl_L] + data.qvel[d_whl_R]) / 2.0) * WHEEL_R
                label += f" | MAX SPD {vel_ms:.2f} m/s"
                rgba0 = [0.2, 0.6, 1.0, 1.0]
            mujoco.mjv_initGeom(
