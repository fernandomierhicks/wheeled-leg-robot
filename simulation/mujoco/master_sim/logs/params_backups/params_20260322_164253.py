...
class RobotGeometry:
    ...
    Q_RET: float = -0.35071         # [rad] fully retracted (crouch)
...
class LQRGains:
    ...
    Q_pitch: float = 0.0679322
    Q_pitch_rate: float = 1.88554
    Q_vel: float = 5.3895e-04
    R: float = 269.789
...
class VelocityPIGains:
    ...
    Kp: float = 0.261275
    Ki: float = 0.411397
    Kff: float = 0.00905434             # [s²·rad/m] ≈ 1/g — feed-forward: lean per unit dv_target/dt
    theta_max: float = 0.8        # [rad] ±46° clamp  Max commandable lean angle.
    int_max: float = 1.0           # [rad·s] anti-windup ( theta_max / Ki)
    theta_ref_rate_limit: float = 50.0  # [rad/s]  How fast can commanded lean angle change every tick (larger number = more aggresive)
...
class SuspensionGains:
    ...
    K_s: float = 61.4754               # [N·m/rad] spring stiffness
    B_s: float = 0.492791              # [N·m·s/rad] damping
    K_roll: float = 9.45444            # [rad/rad] roll proportional
    D_roll: float = 0.00483883           # [rad·s/rad] roll rate damping
...
class LatencyParams:
    ...
    def actuator_delay_steps(self, sim_timestep: float) -> int:
        return round(self.actuator_delay_s / sim_timestep)
...
class ScenarioTimings:
    ...
    @property
    def leg_cycle_Q_RET(self) -> float:
        """S4 crouched setpoint — 10° less crouched than Q_RET."""
        return -0.35071 - math.radians(10.0)
