...
Run:
    python simulation/mujoco/4bar_jump_sim/4bar_jump_sim.py
"""
...
A_Z = -0.0235; F_X = -0.015; F_Z = 0.0025
...
KP =  20.0              # PD proportional gain [N·m/rad]
KD =   1.0              # PD derivative gain   [N·m·s/rad]

MAX_TORQUE =   7.0      # AK45-10 peak torque [N·m]
HOLD_TORQUE =  2.5      # AK45-10 hold torque [N·m]
OMEGA_MAX  = 18.85      # no-load speed at 24V, output shaft [rad/s]  (75KV×24V/10:1=180RPM)
JUMP_RAMP_S    = 0.010  # torque ramp-up duration at jump start [s]
JUMP_RAMPDOWN  = 0.15   # [rad] taper torque to 0 in last 0.15 rad before Q_EXTENDED (~8.6°)
JUMP_SLOWMO    = 20     # slow-motion factor during jump stroke + flight (JUMPING + LANDING)
...
if __name__ == "__main__":
    ...
    CTRL_HZ         = RENDER_HZ                         # control rate in sim-time [Hz]
    steps_per_frame = max(1, PHYSICS_HZ // RENDER_HZ)   # physics steps per render at 1× speed
    steps_per_ctrl  = max(1, PHYSICS_HZ // CTRL_HZ)     # physics steps between control updates
    push_interval   = 1.0 / PUSH_HZ
    last_push_t     = -1.0
    prev_sim_t      = 0.0
    ctrl_step_acc   = 0                                  # counts physics steps since last control update
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            # Control is decoupled: updates every steps_per_ctrl physics steps
            # regardless of how many steps we take per render frame.
            ...
            # ── State machine + torque (runs at CTRL_HZ in sim-time) ──────
            if ctrl_step_acc >= steps_per_ctrl:
                ...
                if state == FALLING:
                    ...
                    ramp_out = min(1.0, max(0.0, (hip_q - Q_EXTENDED) / JUMP_RAMPDOWN))
                ...
                # Push telemetry
                sim_t_now = float(data.time)
                if sim_t_now - last_push_t >= push_interval and not plot_q.full():
                    wheel_z_tel = float(data.xpos[wheel_bid][2])
                    plot_q.put_nowait((sim_t_now, hip_q, torque,
                                       max(0.0, (wheel_z_tel - WHEEL_R) * 1000.0)))
                    last_push_t = sim_t_now
