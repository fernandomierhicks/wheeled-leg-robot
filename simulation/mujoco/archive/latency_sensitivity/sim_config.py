...
LQR_Q_PITCH      =  0.005408  # weight on pitch error
LQR_Q_PITCH_RATE =  0.001177     # weight on pitch rate  — floored at 1e-3 (previous 0.000143 eliminated damping, causing sandbox instability)
LQR_Q_VEL        =  2e-06  # weight on wheel velocity (↓23× vs zero-latency)
LQR_R            =  1100.1681  # weight on control effort (↓14× vs zero-latency)

# ── Leg impedance (held at Q_NOM; decoupled from balance loop) ─────────────
LEG_K_S = 0.388999   # [N·m/rad] spring stiffness  (latency model: 652 gens / 5216 evals, fitness=7.498, 2026-03-19; ↓8× vs zero-latency — stiff spring oscillates with delayed feedback)
LEG_B_S = 5.171191   # [N·m·s/rad] damping        (latency model: ↑5× vs zero-latency — high damping compensates for soft spring)
...
LEG_K_ROLL         = 5.6489  # [rad/rad]     roll proportional gain (latency model: 5216 evals, 2026-03-19)
LEG_D_ROLL         = 0.063168  # [rad·s/rad]   roll rate damping (latency model; zero-latency ref: K=4.0, D=1.0)
...
VELOCITY_PI_KP = 0.003203    # [rad/(m/s)] S5 20-min opt (latency+soft suspension): fitness=6.508, 9312 evals, 2026-03-19
VELOCITY_PI_KI = 20   # [rad/m]     KP dominant with soft suspension — integrator less needed
...
YAW_PI_KP         = 20   # [N·m / (rad/s)] S6 20-min opt (latency model): 1911 gens, 15288 evals, fitness=1.7595, 2026-03-19
YAW_PI_KI         = 1.172637  # [N·m / rad]     KI_YAW↓62% vs zero-latency — integral accumulates too much with stale yaw rate
