...
from run_log import log_run, next_run_id
...
W_SETTLE     = 0.01   # S1/S2: penalty per second of settle time — discourages passive/sluggish gains
...
def _run_sim_loop(model, data, duration: float, v_profile_fn,
    ...
    while data.time < duration:
        if step % CTRL_STEPS == 0:
            ...
            data.ctrl[act_wheel_R] = motor_taper(_tau_R_d, data.qvel[d_whl_R], v_batt)
...
def run_balance_scenario(gains: dict, duration: float = BALANCE_DURATION,
    ...
    # Fitness: S1 style (ISE pitch + pitch rate) instead of wheel travel
    fitness = (
        ise_pitch
...
def run_2_leg_height_gain_sched(gains: dict, duration: float = SCENARIO_4_DURATION,
    ...
    # ── Latency ring buffers (mirrors S1 / _run_sim_loop) ────────────────────
    _n_sens = max(1, round(SENSOR_DELAY_S   / dt)) if USE_LATENCY_MODEL else 1
    _n_act  = max(1, round(ACTUATOR_DELAY_S / dt)) if USE_LATENCY_MODEL else 1
    _pitch0 = get_equilibrium_pitch(ROBOT, Q_NOM)
    _sens_buf = collections.deque([(_pitch0, 0.0, 0.0)] * _n_sens, maxlen=_n_sens)
    _ctrl_buf = collections.deque([(0.0, 0.0)]           * _n_act,  maxlen=_n_act)
    _pitch_d, _pitch_rate_d, _wheel_vel_d = _pitch0, 0.0, 0.0
    ...
    while data.time < duration:
        if step % CTRL_STEPS == 0:
            ...
            # Settle detection (same criterion as S1)
            if not settled:
                if pitch_err_deg < SETTLE_THRESHOLD:
                    if settle_start is None:
                        settle_start = data.time
...
def evaluate(gains: dict, scenario: str = "balance", label: str = "",
             run_id: int = None, csv_path: str = None) -> dict:
    if csv_path is None:
        raise ValueError("evaluate() requires an explicit csv_path — use get_scenario_csv_path(scenario)")
    if scenario == "balance":
