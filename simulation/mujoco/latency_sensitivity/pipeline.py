...
Runs the full 8-step gain chain sequentially and unattended:
  S1: LQR on 1_LQR_pitch_step         (seed: sim_config baseline)
  S2: LQR on 2_leg_height_gain_sched  (seed: S1 best)  -- LQR BASELINED in sim_config.py
  S3: VelPI on 3_VEL_PI_disturbance   (seed: sim_config baseline)
  ...
  S6: YawPI on 6_YAW_PI_turn          (seed: sim_config baseline)
  S7: YawPI on 7_DRIVE_TURN           (seed: S6 best)  -- YawPI BASELINED in sim_config.py
  S8: Susp on 8_terrain_compliance    (seed: sim_config baseline) -- Suspension BASELINED
...
Usage:
    python pipeline.py                          # all 8 steps, 1h each
...
""")
    ...
    _log(f"  patience={args.patience}  tol={args.tol}  fresh={args.fresh}")
    ...
    for i, (step_id, optimizer_script, scenario_name, param_keys, seed_from, baseline_map) in enumerate(active_steps):
        ...
        # Resolve seed gains from prior step
        seed_gains = None
        if seed_from is not None:
            ...
            if seed_gains:
                ...
                _log(f"  WARNING: {seed_from} has no result yet — starting from sim_config baseline")
