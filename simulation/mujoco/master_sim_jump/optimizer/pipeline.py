...
STEPS = [
    ("S1", "master_sim_jump.optimizer.optimize_lqr",
     "s01_lqr_pitch_step",
     ["Q_PITCH", "Q_PITCH_RATE", "Q_VEL", "R"],
     None, False),

    ("S2", "master_sim_jump.optimizer.optimize_lqr",
     "s02_leg_height_gain_sched",
     ["Q_PITCH", "Q_PITCH_RATE", "Q_VEL", "R"],
     "S1", True),

    ("S3", "master_sim_jump.optimizer.optimize_vel_pi",
     "s03_vel_pi_disturbance",
     ["KP_V", "KI_V", "KFF_V"],
     None, False),

    ("S4", "master_sim_jump.optimizer.optimize_vel_pi",
     "s04_vel_pi_staircase",
     ["KP_V", "KI_V", "KFF_V"],
     "S3", False),

    ("S5", "master_sim_jump.optimizer.optimize_vel_pi",
     "s05_vel_pi_leg_cycling",
     ["KP_V", "KI_V", "KFF_V"],
     "S4", True),

    ("S6", "master_sim_jump.optimizer.optimize_yaw_pi",
     "s06_yaw_pi_turn",
     ["KP_YAW", "KI_YAW"],
     None, False),

    ("S7", "master_sim_jump.optimizer.optimize_yaw_pi",
     "s07_drive_turn",
     ["KP_YAW", "KI_YAW"],
     "S6", True),

    ("S8", "master_sim_jump.optimizer.optimize_suspension",
     "s08_terrain_compliance",
     ["LEG_K_S", "LEG_B_S", "LEG_K_ROLL", "LEG_D_ROLL"],
     None, True),

    ("S9", "master_sim_jump.optimizer.optimize_integrated",
     "s09_integrated",
     ["Q_PITCH", "Q_PITCH_RATE", "Q_VEL", "R",
    ...
    ("S10", "master_sim_jump.optimizer.optimize_integrated",
     "s10_jump",
...
def _random_seed(param_keys: list[str]) -> dict:
    ...
    import numpy as np
    from master_sim_jump.optimizer.search_space import INTEGRATED_SPACE
...
def _read_best_from_csv(scenario: str, param_keys: list[str]) -> dict | None:
    ...
    from master_sim_jump.optimizer.run_log import get_scenario_csv_path, load_best_params
