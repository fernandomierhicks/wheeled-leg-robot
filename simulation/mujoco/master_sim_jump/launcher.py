...
# Ensure master_sim_jump is importable when running this file directly
_MUJOCO_DIR = str(Path(__file__).resolve().parent.parent)
if _MUJOCO_DIR not in sys.path:
    sys.path.insert(0, _MUJOCO_DIR)

from master_sim_jump.viz.visualizer import run_unified
...
PIPELINE_STEPS = ["S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8", "S9", "S10"]
...
def _launch_pipeline(start_var, end_var, hours_var, workers_var, patience_var,
    ...
    cmd = [sys.executable, "-m", "master_sim_jump.optimizer.pipeline"]
