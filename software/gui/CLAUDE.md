...
| `tabs/host_logger.py` | `HostLogger` singleton — captures every `TelemetryBus` packet (TELEM, robot LOG lines, CALIB, WIFI_DIAG) to a timestamped `.jsonl`. `LogTransferManager` automatically starts/stops it with every observed Teensy SD logging session, including remote/radio/onboard triggers; manual `host_log_start`/`host_log_stop` remains available for an independent host-only capture. The only way to see firmware `comm_log()` lines and TELEM-arrival timing from outside the GUI process — see "Remote control" below. |
...
| `host_log_start` / `host_log_stop` / `host_log_status` | Manual controls/status for GUI-side capture (`HostLogger`) of *everything* the GUI receives — including firmware `comm_log()` LOG frames — to `data/logs/runs/<ts>_HOST/host.jsonl`. SD logging also starts/stops this capture automatically. **This is how you see firmware log lines and measure real inter-packet timing from a headless shell** — parse the `.jsonl`, filter `type_name == "LOG"` for `log_msg` (e.g. `Loop overrun: ...` warnings), or diff consecutive `TELEM` records' `host_monotonic_ns` to detect a stalled control loop. |
