...
class RawDataTab(QWidget):
    ...
    def _build_control_tab(self, tabs: QTabWidget):
        content, g = self._make_page()
        r = 0
        r = self._section(g, r, "Balance references")
        r = self._add_row(g, r, ("theta_ref",            "—"),  ("v_ref",               "—"))
        r = self._section(g, r, "Torques")
        r = self._add_row(g, r, ("tau_sym",              "—"),  ("tau_yaw",             "—"))
        r = self._section(g, r, "Integrators")
        r = self._add_row(g, r, ("vel_err_integral",     "—"),  ("yaw_err_integral",    "—"))
        r = self._section(g, r, "Feedforward")
        r = self._add_row(g, r, ("ff1_out",              "—"),  ("ff2_out",             "—"))
        r = self._add_row(g, r, ("ff4_out",              "—"),  ("",                    ""))
    ...
    def _on_packet(self, info: dict):
        ...
        self._set("vel_err_integral",    f"{f.get('vel_err_integral', 0.0):+.6f}")
        self._set("yaw_err_integral",    f"{f.get('yaw_err_integral', 0.0):+.6f}")
        self._set("ff1_out",             f"{f.get('ff1_out', 0.0):+.4f}")
        self._set("ff2_out",             f"{f.get('ff2_out', 0.0):+.4f}")
        self._set("ff4_out",             f"{f.get('ff4_out', 0.0):+.4f}")
