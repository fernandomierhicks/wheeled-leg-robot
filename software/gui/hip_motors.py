...
NOTE: cmd_id schema for hip commands is not yet defined in firmware.
      The _build_frame / _send helpers are wired up but marked TODO.
"""
...
# Hip sub-command values (mirrors old CMD_HIP convention)
_HIP_CMD_DISABLE = 0
_HIP_CMD_ENABLE  = 1
_HIP_CMD_ZERO    = 2
_HIP_CMD_MIT     = 3

# TODO: assign a top-level cmd_id once firmware command schema is finalized
_CMD_HIP_MOTOR = 5
...
class _MotorPanel(QWidget):
    ...
    def __init__(self, motor_id: int, title: str, node_hex: str):
        ...
        # ── Presets: fill Kp / Kd ────────────────────────────────────────────
        # (Kp max=500 N·m/rad, Kd max=5.0 N·m·s/rad per AK45 MIT frame limits)
        _PRESETS = [
    ...
    def _simple_cmd(self, motor_id: int, hip_sub: int):
        # TODO: replace _CMD_HIP_MOTOR with the final firmware cmd_id
        payload = struct.pack("<BB", _CMD_HIP_MOTOR, 0) + struct.pack("<BB", motor_id, hip_sub)
    ...
    def _send_mit(self):
        ...
        payload = struct.pack("<BBBfffff",
                              _CMD_HIP_MOTOR, self._mid, _HIP_CMD_MIT,
                              p_rad, 0.0,
...
class HipMotorsTab(QWidget):
    ...
    def _both_cmd(self, hip_sub: int):
        # motor_id 0 → broadcast to both (firmware convention TBD)
        payload = struct.pack("<BBB", _CMD_HIP_MOTOR, 0, hip_sub)
        _send(_build_frame(payload))
