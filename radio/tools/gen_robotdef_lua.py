...
GEN_PROTOCOL = (Path(__file__).resolve().parents[2] / "firmware" / "robot_teensy"
                / "protocol" / "generate_protocol.py")
...
def flightmode_names():
    """The short fault names the firmware puts in the CRSF FLIGHT_MODE frame.

    Parsed out of generate_protocol.py rather than duplicated, because the
    radio has to map that text back to a fault code when the private telemetry
    frame does not relay -- and a mismatch there means the radio hears a fault
    it cannot name.
    """
    src = GEN_PROTOCOL.read_text(encoding="utf-8")
    m = re.search(r"FAULT_FLIGHTMODE_NAMES\s*=\s*\{(.*?)\n\}", src, re.S)
    if not m:
        sys.exit("could not find FAULT_FLIGHTMODE_NAMES in %s" % GEN_PROTOCOL)
    out = {}
    for sym, name in re.findall(r'"(FAULT_\w+)":\s*"([^"]*)"', m.group(1)):
        out[sym] = name
    if not out:
        sys.exit("FAULT_FLIGHTMODE_NAMES parsed empty")
    return out
...
def render(states, faults, fm):
    ...
    w("}")
    w("")
    w("-- Reverse maps for the CRSF FLIGHT_MODE text sensor. State and fault")
    w("-- reach the radio twice: as numbers in the private 0x24 frame, and as")
    w("-- text here. The text is the half guaranteed to relay through ELRS, so")
    w("-- these let the HUD and the annunciator keep working on it alone.")
    w("M.fmState = {")
    ...
    w("M.fmFault = {")
    for f in sorted(faults, key=lambda f: f["id"]):
        if f["symbol"] == "FAULT_NONE":
            continue
        w('  ["%s"] = %d,' % (fm[f["symbol"]], f["id"]))
    w("}")
    w("")
...
def main():
    ...
    fm = flightmode_names()
    missing = sorted(f["symbol"] for f in faults if f["symbol"] not in fm)
    if missing:
        sys.exit("generate_protocol.py has no FLIGHT_MODE name for: %s" % missing)
    text = render(states, faults, fm)
