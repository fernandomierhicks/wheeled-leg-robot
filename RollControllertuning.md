...
- Existing hard clamps (`PARAM_LQR_TORQUE_LIMIT`, `PARAM_WHEEL_VEL_LIMIT_TURNS_S` + its 2× hard-fault multiplier, the pitch watchdog (`pitch_wd_fwd/bwd_ret/ext`, asymmetric + gain-scheduled), `MAX_HIP_DELTA_RAD`, and the vel-PI (`theta_max_fwd/bwd_ret/ext`) / yaw-PI clamp params) stay exactly as configured throughout — the search only ever varies the stage's own Kp/Ki (or K_pitch/K_rate) pair, never loosens a limit to "let a candidate through." They apply identically regardless of whether `v_cmd_ms`/`omega_cmd_rds` came from radio or the GUI motion channel.
