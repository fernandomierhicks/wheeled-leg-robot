...
static int opProcess(sh2_t *pSh2, const sh2_Op_t *pOp)
{
    ...
    // No sh2_Op_t in this file sets timeout_us, so without this fallback
    // opProcess() never bails out at all when a request's response never
    // arrives (e.g. getProdIdOp during sh2_getProdIds()) — it busy-loops
    // forever with no way for the caller to recover from a non-responding
    // sensor (loose/dead SPI connection). Measured on real hardware (test_imu
    // SH2 DEBUG trace): sh2_getProdIds() alone legitimately takes ~690 ms on
    // a healthy connection — a prior version of this patch used a 100 ms
    // fallback ("a live sensor responds within a few ms"), which was wrong
    // and silently timed out every real connection attempt before the sensor
    // finished responding. 3 s gives a healthy sensor comfortable margin
    // while still bounding a truly dead/disconnected one.
    static const uint32_t DEFAULT_OP_TIMEOUT_US = 3000000UL;
    uint32_t timeout_us = (pOp->timeout_us != 0) ? pOp->timeout_us : DEFAULT_OP_TIMEOUT_US;
    ...
    while ((pSh2->pOp != 0) && ((now_us-start_us) < timeout_us)) {
