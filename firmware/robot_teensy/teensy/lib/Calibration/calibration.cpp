...
// Every phase treats a sustained over-torque as "something jammed" and faults
// the axis. Calibration only ever moves between the retract switch and the
// backoff pose, so there is no phase where torque is a measurement rather than
// a fault — see the note above define_limits.
...
// the switch-zero frame, and comes from the configured calib_range_*_rad.
//
// Calibration deliberately does NOT drive out to the extended stop to measure
// it. It once did (a SEEK_EXTENDED phase that pushed into the stop and read the
// torque trip as the measurement), and that was removed: the robot is unbalanced
// and unsupported through calibration, so swinging the leg through its full
// stroke there is not a safe way to learn a number the mechanism already fixes.
// The travel from the retract switch to the extended stop is a property of the
// linkage (~85 deg on v4) — trust it, and keep calibration's motion confined to
// the switch and the small backoff next to it.
    ...
    switch (ax.state) {
        ...
        case CAL_BACKOFF: {
            ...
            if (actual_travel >= backoff) {
                ...
                comm_log(LOG_LEVEL_INFO,
                         "Calib %s: backoff %.1f deg complete",
                         tag, backoff * 57.2957795f);

                // Backoff IS the rest pose, so there is nothing to walk back
                // from — define the span from the configured range and ramp
                // down right here.
                define_limits(ax, limits, away_dir * range_rad);
                comm_log(LOG_LEVEL_INFO,
