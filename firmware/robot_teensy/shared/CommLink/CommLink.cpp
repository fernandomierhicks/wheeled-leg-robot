...
bool CommLink::send(uint8_t type, uint8_t version, const void* payload, uint16_t len,
                    uint8_t corrupt_mode_for_test) {
    if (len > COMM_MAX_PAYLOAD) return false;  // overflow guard — caller passed oversized payload
