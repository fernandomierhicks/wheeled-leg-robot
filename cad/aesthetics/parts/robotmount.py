r"""RobotMount v4 -- the plate that carries the leg module into the box.
...
PART = "RobotMount"
B, S = (151.0, 33.0), (100.653, 44.544)      # derived; see module docstring
RAD = {B: 13.45, S: 8.3}   # each datum's own feature radius
...
def plan(sp):
    ...
    strip, blocks, ring, channel = ACC.accents(
        ...
        joints=[(p, RAD[p] + 2.5) for p in (B, S)])
...
if __name__ == "__main__":
    sf = os.path.join(paths.SPECS, "robotmount.json")
    sp = spec_mod.derive()
