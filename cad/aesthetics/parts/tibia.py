r"""Tibia v4 -- 'Arctic' styling recipe.

Two entry points, one spec:

    plan(spec)   -> pure shapely/2D.  Cheap.  Feeds the board AND the build.
    build(spec)  -> OCC solids, per-filament bodies, 3MF.  Expensive.

Nothing is reinterpreted between them, so what a board cell shows is what gets built.

EVERY opening in the source (bores, counterbores, mounting holes, zip-tie slots) is
protected: material is only ever ADDED outside the original silhouette, and the
source solid is never re-cut.
"""
import os, sys, json
...
PART = "Tibia"
E, C, W = (0.0, 0.0), (38.65, -5.28), (224.49, 0.0)     # datums, agree with CLAUDE.md
SHOW_FACE = "+Z"   # the Tibia is NOT mirrored: the assembly walk puts its
                   # local +Z outboard already.  Declared so the shared
...
def source():
    """Load and cache the source solid plus its invariants."""
    if "solid" not in _CACHE:
        s = import_step(paths.part_step(PART)).solids()[0]
        bb = s.bounding_box()
        op = openings(s, bb.max.Z, bb.min.Z)
        _CACHE.update(solid=s, ZT=bb.max.Z, ZB=bb.min.Z, OUT=silhouette(s),
                      OPEN=op, KEEP=unary_union(op))
    return _CACHE
...
def _slots(x0, x1, n, gap=9.0):
    """Split [x0,x1] into n runs.  The gap shrinks as n rises, otherwise a dense
    language asks for 13 features and a fixed gap leaves no room for any."""
    if n <= 0: return []
    span = x1 - x0
...
def _edge_y(poly, x, side):
    ys = _yspan(poly, x)
    return None if ys is None else (ys[1] if side > 0 else ys[0])


def _trap_edge(poly, segs, side=1, ramp=14.0):
    """Trapezoidal growth along one edge: flat plateaus at an ABSOLUTE Y, joined
    by angled ramps, sharp vertices throughout.

    Offsetting the source edge by a constant would inherit the source's curves;
    holding the plateau at an absolute Y is what makes the run dead straight,
    which is what "sharper creases / trapezoidal lines" asks for."""
    sgn = 1.0 if side > 0 else -1.0
    out = []
    for xa, xb, h in segs:
        if h <= 0.05 or (xb - xa) < 2 * ramp + 4:
            continue
        base, ya, yb = None, _edge_y(poly, xa, side), _edge_y(poly, xb, side)
        for x in np.linspace(xa, xb, 28):
            y = _edge_y(poly, x, side)
            if y is None: continue
            base = y if base is None else (max(base, y) if side > 0 else min(base, y))
        if base is None or ya is None or yb is None:
            continue
        top = base + sgn * h
        out.append(ShPoly([(xa, ya), (xa + ramp, top), (xb - ramp, top), (xb, yb),
                           (xb, 0.0), (xa, 0.0)]))
    return unary_union(out) if out else ShPoly()


def _collars(sp, grown, kb, knee, extra):
    """Graphite surface bodies: the bore collars, plus anything handed in via
    `extra` (the accent channel).  `collar_t` slims them, `collar_seg` facets the
    OUTER edge only -- the inner edge is the bore and can never move."""
    ct, cseg = sp.get("collar_t", 3.4), int(sp.get("collar_seg", 96))
    ntab, tabs = int(sp.get("collar_tabs", 0)), []
    cr = [(E, 17.2 + ct + sp["knee_grow"], 17.2),
          (C, 17.2 + ct + sp["knee_grow"], 17.2),
          (W, 26.9 + max(ct - 1.3, 1.2) + sp["wheel_grow"], 26.9)]
    # A faceted buffer INSCRIBES its polygon, so an octagon at radius ro has its
    # flats at ro*cos(pi/8) -- inside the bore, shattering the ring into slivers.
    fk = 1.0 / np.cos(np.pi / (4 * cseg)) if cseg < 8 else 1.0
    out = unary_union([ShPoint(*p).buffer(ro * fk, cseg).difference(ShPoint(*p).buffer(ri, 96))
                       for p, ro, ri in cr])
    if ntab > 0:
        for (px, py), ro, _ in cr:
            for k in range(ntab):
                th = np.pi / 4 + k * 2 * np.pi / ntab
                cx, cy = px + ro * fk * np.cos(th), py + ro * fk * np.sin(th)
                t = ro * 0.30
                tabs.append(ShPoly([(cx - t, cy - t), (cx + t, cy - t),
                                    (cx + t, cy + t), (cx - t, cy + t)]))
        out = unary_union([out] + tabs)
    nr, rings2 = int(sp.get("n_rings", 0)), []
    if nr > 0:
        rr, rt = sp.get("ring_r", 7.0), sp.get("ring_t", 1.7)
        band = sp.get("ring_band", "mid")
        xs = np.linspace(152, 200, nr) if band == "cluster" else np.linspace(74, 196, nr)
        for i, x in enumerate(xs):
            ys = _yspan(grown, x)
            if ys is None: continue
            lo, hi = ys
            r = rr * (1.4 if (band == "mixed" and i % 3 == 0) else 1.0)
            if band == "upper":   yc = hi - r - 4.5
            elif band == "mixed": yc = (lo + hi) / 2 + (r * 0.95 if i % 2 else -r * 0.95)
            else:                 yc = (lo + hi) / 2
            rings2.append(ShPoint(x, yc).buffer(r, 64)
                          .difference(ShPoint(x, yc).buffer(max(r - rt, 0.5), 64)))
        if rings2:
            out = unary_union([out, unary_union(rings2)])
    if not extra.is_empty:
        out = unary_union([out, extra])
    return out.intersection(grown).difference(kb)


def plan(sp):
    """All 2D geometry for a spec.  No OCC, no booleans on solids."""
    src = source()
    ...
    knee = unary_union([ShPoint(*E).buffer(21.0 + sp["knee_grow"], 96),
                        ShPoint(*C).buffer(21.0 + sp["knee_grow"], 96)]).convex_hull
    wheel = unary_union([ShPoint(*W).buffer(23.5 + sp["wheel_grow"], 64),
                         OUT.intersection(shbox(196, -90, 260, 90))]).convex_hull
    if facet:                       # straight-facet blocks
        knee, wheel = knee.simplify(1.1), wheel.simplify(1.4)
    else:                           # smooth: keep the lobes round, no hull corners
        knee = unary_union([ShPoint(*E).buffer(21.0 + sp["knee_grow"], 128),
                            ShPoint(*C).buffer(21.0 + sp["knee_grow"], 128)]).buffer(2.0).buffer(-2.0)
        wheel = wheel.buffer(3.0).buffer(-3.0)
    kd = sp["keel_depth"]
    prof = ([(56, 0), (88, .84), (122, 1.0), (156, .84), (190, 0)] if facet else
            [(56, 0), (72, .55), (88, .84), (105, .97), (122, 1.0), (139, .97),
             (156, .84), (173, .55), (190, 0)])
    keel = ShPoly([(x, _yspan(OUT, x)[0] - kd * f) for x, f in prof] + [(190, 0), (56, 0)])
    if not facet:
        keel = keel.buffer(3.5).buffer(-3.5)
    # Trapezoidal edge growth.  Straight plateaus, sharp vertices, on the upper
    # edge and (optionally) the lower one, so the change reads as creased panels
    # rather than as a bulge.  Pure addition outside the source silhouette, so
    # the bores are never approached.
    oh, ramp = sp.get("out_h", 0.0), sp.get("out_ramp", 14.0)
    nstep, sides = sp.get("out_steps", 2), sp.get("out_sides", "both")
    dorsal = ShPoly()
    if oh > 0.05:
        if nstep >= 2:
            top_segs = [(58, 134, oh * 0.68), (128, 208, oh)]
            bot_segs = [(56, 138, oh * 0.85), (132, 200, oh * 0.55)]
        else:
            top_segs = [(58, 208, oh)]
            bot_segs = [(56, 200, oh * 0.8)]
        bands = [_trap_edge(OUT, top_segs, side=1, ramp=ramp)]
        if sides == "both":
            bands.append(_trap_edge(OUT, bot_segs, side=-1, ramp=ramp))
        dorsal = unary_union([b for b in bands if not b.is_empty])
    g0 = unary_union([OUT, knee, wheel, keel, dorsal]).buffer(0)
    if sp["notches"]:
        # The cut has to run clear of the surface.  Stopping 0.3 mm short of it
        # leaves a razor ledge, which against a flat plateau becomes a 0.4 deg
        # spike that collapses OCC's chamfer taper.
        nt = unary_union([ShPoly([(x - 7, _yspan(g0, x)[1] + 2.5), (x + 7, _yspan(g0, x)[1] + 2.5),
                                  (x, _yspan(g0, x)[1] - 6.0)]) for x in (76, 206)])
        g0 = g0.difference(nt)
    # Organic mode: round every hard vertex into a tangent curve and neck the
    # mid-shaft in, so the part reads as the concept art's waisted link with
    # circular bosses rather than a constant-width slab with trapezoid steps.
    orr = sp.get("org_r", 0.0)
    if orr > 0.05:
        # close then open: the close fills the crease notches, the open rounds
        # every convex corner.  Round joins throughout -- mitre would put the
        # hard vertices straight back.
        g1 = (g0.buffer(orr, join_style=1).buffer(-2 * orr, join_style=1)
                .buffer(orr, join_style=1))
        wd = sp.get("waist_d", 0.0)
        if wd > 0.05:
            # Two very large circles, one above and one below, cut a smooth
            # concave neck.  Large radius is what makes the waist read as a
            # tangent curve instead of a bite.
            R, xmid = 165.0, 132.0
            ys = _yspan(g0, xmid)
            if ys is not None:
                lo, hi = ys
        ...
        # NEVER end up inside the source: re-union guarantees the waist can only
        # ever eat material this layer added, never the original part.
        g0 = unary_union([g1, OUT]).buffer(0)
        g0 = max(geoms(g0), key=lambda g: g.area)
    # Organic mode's round-join buffers leave a lot of sub-millimetre edges, and
    # offsetting such a wire by the 4.4 mm chamfer makes OCC fail with
    # "BRepFill_OffsetWire::FixHoles(): Wrong wire".  0.5 mm of simplification is
    # invisible on a 270 mm part and removes the problem at source.
    ...
    # The envelope gets tapered at 45 deg for the chamfer, and OCC's taper dies on
    # razor slivers and on enclosed voids.  Unioning bands onto the source outline
    # can produce both.  Dropping interior rings is safe here because `grown` only
    # ever CLIPS the body -- material comes from the source solid and from `add`,
    # which is differenced against KEEP, so no bore can be filled by this.
    ...
    # cannot grow where the links overlap in plan: they clear each other by
    # interleaving in DEPTH, so one contested region blocks the whole thickness.
    # Features live on the show face, so `grown` becomes the OUTERMOST layer.
    ...
    # raised: keel armour frame + upper rail
    frame = (grown.buffer(-3.6, join_style=2).intersection(shbox(64, -90, 192, -14.0))
             .difference(kb).difference(knee.buffer(1.5)).difference(wheel.buffer(1.5)))
    ...
    rail = (grown.buffer(-2.6, join_style=2).difference(grown.buffer(-7.2, join_style=2))
            .intersection(shbox(60, 9.0, 196, 90)).difference(kb))
    rail = unary_union([g for g in geoms(rail) if g.area > 20]) if sp["rail_h"] > 0.3 else ShPoly()

    # raised pads, alternating keel band / upper band.  pad_size sets how tall
    # each one is, so 'armored' gets a few big plates and 'greebled' many small.
    ps = sp.get("pad_size", 1.0)
    pads = []
    gk = sp.get("gap_k", 1.0)
    for i, (a, b) in enumerate(_slots(66, 194, sp["n_pads"], 8.0 * gk)):
        lo, hi = _yspan(grown, (a + b) / 2)
        if i % 2 == 0:  pads.append(trap_plan(a, b, lo + 1.0, lo + 1.0 + 8.0 * ps, 7.0, 1.4))
        else:           pads.append(trap_plan(a, b, hi - 9.5 - 6.0 * ps, hi - 9.5, 7.0, -0.9))
    pads = clip(unary_union(pads), 3.0) if pads else ShPoly()

    # recessed pockets
    qs = sp.get("pock_size", 1.0)
    pock = []
    for i, (a, b) in enumerate(_slots(62, 188, sp["n_pockets"], 7.0 * gk)):
        lo, hi = _yspan(grown, (a + b) / 2)
        if i % 2 == 0:  pock.append(trap_plan(a, b, lo + 11.0, lo + 11.0 + 5.0 * qs, 6.0, 1.2))
        else:           pock.append(trap_plan(a, b, hi - 17.5 - 4.5 * qs, hi - 17.5, 6.0, -0.7))
    pock = clip(unary_union(pock)) if pock else ShPoly()

    # through cutouts -- the 'exposed' language removes material instead of adding
    cuts = []
    for a, b in _slots(70, 192, sp.get("n_cuts", 0), 11.0):
        lo, hi = _yspan(grown, (a + b) / 2)
        top = lo + 6.0 + max(6.0, kd * 0.75)
        cuts.append(trap_plan(a, b, lo + 5.0, top, 6.0, 1.0))
    cutouts = clip(unary_union(cuts), 4.5) if cuts else ShPoly()

    # blind recesses inside the keel frame
    wins = []
    for a, b in _slots(86, 184, sp["n_wins"], 8.0):
        lo, _ = _yspan(grown, (a + b) / 2)
        wins.append(trap_plan(a, b, lo + 3.0, lo + 10.0, 5.0, 1.0))
    ...
    # empty space and the board stops being a contract
    # The blue used to be several disconnected 1.8 mm slivers at a fixed Y plus
    # a 1.8 mm ring -- every blue element was a hairline, and they shared no
    # geometry, so they read as random lines.  Now there is ONE spine that
    # follows the part's own upper contour, blocks strung along that spine, and
    # a wheel ring of the same weight that terminates it.  One line, one logic.
    # the accent vocabulary lives in lib/accents.py so every part shares it
...
def build(sp, verbose=True):
    """Realise a spec as OCC solids + per-filament bodies."""
    P = plan(sp)
    ...
    # One envelope and one addition per z layer.  A tapered extrude is the one
    # operation here that can fail on an otherwise valid outline; losing the
    # chamfer is cosmetic, losing the build is not, so it falls back per layer.
    # With a single layer this is exactly what it always was.
    ...
    for _i, (_z0, _z1, _gk, _mk) in enumerate(layers):
        ...
        if CH > 0.05:
            try:
                ...
                say(f"  chamfer taper failed on layer {_i} "
                    f"({type(e).__name__}); building square-edged")
                _pz = None
        if _pz is None:
        ...
        _a = _gk.difference(P["OUT"]).buffer(0.25).difference(KEEP)
        if not _a.is_empty:
    ...
    # colour: split plane with interlocking trapezoidal fingers.
    # Accent and collar geometry first -- the split is MEASURED, so everything
    # that eats into white has to exist before it can be solved for.
    acc_poly = unary_union([g for g in (P["ring"], P["strip"], P["blocks"])
                            if not g.is_empty])
    ...
    def _colour(SZ):
        wz = slab(SZ, ZTOP + 20)
        for p in [trap_xz(a, b, SZ - 4.5, SZ, skew=6) for a, b in [(58, 102), (138, 182)]]:
            wz = union(wz, full_y(p))
        gz = None
        for p in [trap_xz(a, b, SZ, SZ + 14.5, skew=6, taper_end=False)
    ...
    # `split_z` is an absolute z and does not transfer between parts; `white_share`
    # states the target and solves for the plane.  See parts/femur.py _split_z.
    target = sp.get("white_share")
    if target is None:
        white, blue, graph, tot = _colour(sp["split_z"])
    else:
...
if __name__ == "__main__":
    sf = os.path.join(paths.SPECS, "tibia.json")
    # merge a locked spec OVER current defaults, so a spec locked today keeps
    # working after new fields are added later -- missing keys just take their
    # default instead of raising
    sp = spec_mod.derive()
    if os.path.exists(sf):
        saved = json.load(open(sf))
        sp = spec_mod.derive(**{k: v for k, v in saved.items()
                                if k in ("aggression", "density", "relief",
                                         "scale", "accent", "outline", "organic",
                                         "waist", "language", "palette", "mix")})
        sp.update(saved)
