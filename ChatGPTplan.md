...
| 1 — behavior baselines and metrics | **Complete** | Frozen cross-language frame vectors and ordered FSM contract; Windows-native CommLink suite now covers noise, truncation, bad length/CRC/end, short writes, max payload, overlapping magic, sequence wrap, and recovery (10/10 passed). GUI suite: 18/18 passed. Added a repeatable operator-API link baseline; post-flash all three GUI sources delivered fresh STANDBY/NONE telemetry and parameter transactions, with 116 parameters read back and all four motor enables `0`. Baseline exposed the expected shared-sequencer defect (ESP32/Wi-Fi GUI sequence gaps grow while CRC/pair errors stay zero) and Wi-Fi jitter (up to 200 ms sample age and one 388 ms command), now assigned to Phases 3–4. Teensy COM12 upload succeeded on retry after one loader write failure; ESP32 COM14 upload succeeded. GUI screenshot visually confirmed live data and connection, with the existing degraded-link banner matching these measured deficiencies. Implementation commit `04ad556`. |
| 2 — command safety and state transitions | **Complete** | Added strict shared structural validation for every command (exact lengths, versions, IDs, enums/targets, finite floats), atomic non-finite parameter rejection, v2 request envelopes and correlated command results, retry deduplication, explicit firmware rejection reasons, fresh/nominal IMU arming admission, consumed requests, and `DISARMING` with tested aborts from every energetic state. ESTOP now cuts outputs immediately while ordinary disarm owns the hip ramp. Native suites: CommLink 10/10 and command validation 4/4; GUI/protocol/FSM suite 21/21. Both release builds passed; Teensy COM12 flashed successfully on the second loader attempt and ESP32 COM14 flashed successfully. After reboot all three GUI sources had fresh STANDBY/NONE telemetry, parameter reads succeeded on each source, all four motor-enable parameters read `0`, invalid direct `DISARMING` returned a correlated `INVALID_TARGET` NACK, and repeated ESTOP followed by reset returned cleanly to STANDBY. Per-source samples showed no new CRC/pair drops and command RTT maxima of 60 ms Teensy, 93 ms ESP32, and 163 ms Wi-Fi; the known ESP32/Wi-Fi sequence-counter ownership defect remains assigned to Phase 3 and Wi-Fi latency to Phase 4. GUI screenshot confirmed live fields and command logs. Implementation commit `a48dcae`. |
| 3 — UART ownership and ESP32 scheduling | **Complete** | Consolidated ESP32 output into one writer task and one `CommLink` sequence generator per physical USB, TCP, and UDP stream. Added bounded high-priority control, ACK-paced log, and lossy telemetry queues; byte-budgeted UART/host parsers; non-blocking TCP writes, TCP_NODELAY, and a 250 ms stalled-peer disconnect. Removed the second bulk writer and all post-start raw serial writes. Native CommLink suite 11/11 (including budget-boundary resume), GUI suite 25/25 (including static ownership/priority guards), and both release builds passed. Teensy COM12 flashed on the second loader attempt and ESP32 COM14 flashed successfully. Across a three-source hardware run, new USB/Wi-Fi sequence-gap, UART-gap, UART-drop, and CRC-drop deltas were all zero; the former ESP32/Wi-Fi sequence counters stopped growing. Twelve parameter transactions during a timed recording succeeded, the new 151,134-byte log listed and downloaded over ESP32 USB with transfer CRC success, and a post-transfer three-source run again had zero new sequence/UART/CRC drops. Split Wi-Fi jitter/latency remains explicitly assigned to Phase 4. Implementation commit `c948128`. |
| 4 — Wi-Fi session and traffic policy | **Complete** | Replaced UDP broadcast/split frames with explicit discovery and a renewable single-client unicast lease, one 247-byte combined telemetry datagram at 50 Hz, independent UDP/TCP liveness, no TCP telemetry duplication, TCP_NODELAY, and owner-preserving rejection of competing discovery/TCP clients. GUI live telemetry is coalesced and hidden tabs no longer consume high-rate updates; capture/load tools now claim leases and use v2 commands, and a standalone transport benchmark separates network performance from Qt/API rendering. A 30 s hardware run delivered exactly 1,500 telemetry frames (50.0 Hz), 150 diagnostics, zero gaps/malformed datagrams, telemetry interval p99 35.9 ms (max 93.4 ms), and 300/300 correlated command results with 33.9 ms RTT p99 (max 37.8 ms). GUI restart reacquired all sources automatically; a competing token received `WLR_BUSY_V1`, a second TCP client was closed without evicting the GUI, and post-flash parameter reads succeeded over Teensy USB, ESP32 USB, and Wi-Fi. Native CommLink 11/11, command validation 4/4, and GUI 28/28 passed; both release builds passed. Teensy COM12 flashed on the second loader attempt and ESP32 COM14 flashed successfully. Post-flash counter deltas for CRC, pairing, sequence gaps, and UART drops were all zero, all four motor enables remained `0`, and the GUI showed live STANDBY/NONE data. A physical AP outage/DHCP reassignment was not forced because it would disrupt the host network; ESP32 reboot/flash and GUI-process restart covered endpoint loss and rediscovery. Implementation commit `5859123`. |
| 5 — generated protocol and durable configuration | **Complete** | Added a validated canonical JSON schema for 10 states, 13 faults, 11 commands, 7 parameter groups, and 124 parameters, with checked-in generated C++ IDs/tables, pure-Python GUI definitions, protocol documentation, and vectors. Removed competing definitions from firmware, GUI tabs, and headless tools; `--check` prevents generated drift and GUI tests verify ID/name uniqueness. Replaced the legacy single parameter file with two generation-numbered CRC32 slots, verified writes before advancing, retained the previous valid copy, migrated legacy data, preserved dirty state after save failure, and made firmware-owned `calib_done` genuinely persistent. Tests passed: GUI 32/32, CommLink 11/11, command validation 4/4, parameter-store corruption/wrap 3/3, and both release builds. Teensy COM12 and ESP32 COM14 flashed successfully (Teensy required the usual second loader attempt). Hardware migrated 84 legacy values to slot A, wrote 85 values to slot B generation 2, then rebooted and loaded slot B with both slots valid (`valid_mask=0x03`). All three GUI sources returned fresh STANDBY/NONE telemetry and successful parameter reads; all four motor enables remained `0`, Wi-Fi counters were stable during the sample, and the live GUI was visually confirmed. Implementation commit `8345f39`. |
